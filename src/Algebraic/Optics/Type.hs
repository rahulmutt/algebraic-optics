...
type Getter f n s a = GetterM f Identity n s a -- forall m. (IxMonadState m) => Optic' m f n s s a a
type GetterM f m n s a = forall im. (IxMonadState im, IxMonadLift m im) => Optic' im f n s s a a

type Setter n s t a b = SetterM Identity n s t a b -- forall m. (IxMonadState m) => Optic' m First n s t a b
...
type Traversal' n s a = Traversal n s s a a
type Traversal n s t a b = forall m f. (IxMonadState m, Monoid1 f) => Optic' m f n s t a b
