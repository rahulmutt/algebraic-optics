...
mlens :: forall m s t a b. HasEquality s t a b => (s -> m a) -> (s -> a -> Bool -> m s) -> (s -> b -> m t) -> LensM m s t a b
...
mlens' :: (s -> m a) -> (s -> b -> Bool -> m t) -> LensM m s t a b
...
prefLens :: HasEquality s t a b => (s -> IORef a) -> (s -> IORef b -> t) -> LensIO s t a b
